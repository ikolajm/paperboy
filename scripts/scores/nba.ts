 ...
 * NBA score parsing.
 *
 * Extracts from ESPN NBA scoreboard:
 * - Completed games: scores, leaders (pts/reb/ast), headlines, linescores (quarters)
 * - Scheduled games: start time, broadcasts, venue
 * - Context: play-in, playoff round, season type
 */
...
function parseNotable(event: Record<string, unknown>, homeScore: number, awayScore: number): string | null {
  ...
  // Blowout (30+ point margin)
  const margin = Math.abs(homeScore - awayScore);
  if (margin >= 30) return `Blowout (${margin}-point margin)`;
...
export function parseScheduledGames(data: unknown, targetDate: Date): ScheduledGame[] {
  ...
  for (const event of events) {
    ...
    // Skip games more than 7 days out (season placeholders)
...
export function detectSeasonStatus(data: unknown): { status: "games_played" | "no_games" | "playoffs"; seasonType: number } {
  ...
  // season.type: 1=preseason, 2=regular, 3=postseason, 4=offseason, 5=play-in
  ...
  if (seasonType === 3 || seasonType === 5) return { status: "playoffs", seasonType };
