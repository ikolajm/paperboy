/**
 * College Football score parsing.
 *
 * Same structure as NFL but with college-specific context:
 * - Notes include bowl game names, CFP round, conference championship
 * - Heavily seasonal — offseason returns no events
 */
...
function parseNotable(event: Record<string, unknown>, homeScore: number, awayScore: number): string | null {
  ...
  if (margin >= 28) return `Blowout (${margin}-point margin)`;
  return null;
}
