 ...
 * NFL score parsing.
 *
 * Extracts from ESPN NFL scoreboard:
 * - Completed games: scores, linescores (quarters), headlines
 * - Scheduled games: start time, broadcasts, venue
 * - Context: playoff round, Super Bowl, conference championship
 * - Notable: OT games, blowouts (21+ margin)
 *
 * Note: NFL is seasonal — offseason returns no events.
 * The scoreboard without a date param shows the most recent or upcoming week.
 */
...
function parseNotable(event: Record<string, unknown>, homeScore: number, awayScore: number): string | null {
  ...
  if (margin >= 21) return `Blowout (${margin}-point margin)`;
